...
/*
 * 
 * @ClassName: ScoreFacadeServiceImpl
 * @author 曾智宏
 * @date 2019年5月25日
 * @Description: 该类用于写评分控件的 相关业务代码，充当业务层
 */
@Component
public class ScoreFacadeServiceImpl implements ScoreFacadeService {
	private static final Logger log = LoggerFactory.getLogger(ScoreFacadeServiceImpl.class);



	@Autowired
	private ScoreInfoDao sid;
	...
	public ReturnMsg saveScoreInfo(ScoreInfo scoreInfo) {
		...
		//判断数据库是否已存在该记录
		Integer count = sid.getCountByNodeIdAndSelectId(scoreInfo);

		Integer flat;
		Integer id = null;
		if(count==0) {//如果不存在，就执行保存
			id = sid.saveScoreInfo(scoreInfo);
			System.out.println("id="+id);
			if(id==null)throw new ServiceException("评分信息保存失败");
			flat = 1;
		}else {//执行修改
			Integer row = sid.updateScoreResultByNodeIdAndSelectId(scoreInfo);
	...
	public ReturnMsg deleteScoreInfo(List<Integer> ids) {
		...
		for(Integer id:ids) {
			//删除scoreInfo
			Integer row = sid.deleteScoreInfoById(id);
	...
	public ReturnMsg getScoreResultToOneRecord(Integer nodeId, Integer selectId) {
		...
		Integer scoreResult = sid.getScoreResultByNodeIdAndSelectId(scoreInfo);
	...
	public ReturnMsg getScoreResultAndRecordCount(Integer nodeId) {	
		...
		//查询该数据源表名
		String tableName = sid.getSourceNameBySourceNodeId(nodeId);
		//查询各分人数/记录数
		for(int i=1;i<=5;i++) {
			//定义要查询的评分
			scoreResult = i;
			...
			List<Integer> ids = sid.getIdsByNodeIdAndScoreResult(nodeId,scoreResult);
			...
			for(Integer id:ids) {
				Integer tid = sid.getIdOfNotDelete(id,tableName);
				if(tid!=null) {
					ids2.add(tid);
				}
			}
		...
		//查询数据表的记录数
		Integer allCount = sid.getResultCountByTableName(tableName);
	...
	public ReturnMsg getSelectIdByNodeIdAndscoreResult(Integer nodeId, Integer scoreResult) {
		...
		List<Integer> selectIdList = sid.getSelectIdByNodeIdAndscoreResult(scoreInfo);
	...
	public ReturnMsg getCountsByStatisticalMaterials(List<StatisticalMaterial> smList) {
		...
		for(StatisticalMaterial sm : smList) {
			...
			if(StringUtils.isBlank(statisticsTitle))throw new  ServiceException("title不能为空");
			...
			for(Integer sourceNodeId: sourceNodeIds) {
				String tableName = sid.getSourceNameBySourceNodeId(sourceNodeId);
				if(StringUtils.isBlank(tableName))throw new  ServiceException("查询数据表失败");

				//查询数据表的记录数
				Integer count = sid.getResultCountByTableName(tableName);
