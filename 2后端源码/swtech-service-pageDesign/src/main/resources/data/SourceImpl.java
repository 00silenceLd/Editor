...
public class {className}MapperImpl extends BaseDaoImpl<{className}> implements {className}Mapper { 
	 ...
	 *  @return long
	 */
	public long updateByPrimaryKeySelective({className} entity) {
		try {
			log.info(entity.toString());
			String resource ="{className1}/{className}-mybatis-config.xml";		    
			InputStream inputStream = null;
			...
			mpr.updateByPrimaryKeySelective(entity);
		} catch (Exception e) {
			log.info("出错" + e.toString());
		}
		return 0;
	...
	*/
	 ...
	 * 查询数据总数
	 */
	public long getCountByQuery(Map<String, Object> map) {
		long i = 0;
		try {
			String resource ="{className1}/{className}-mybatis-config.xml";		    
			InputStream inputStream = null;
