...
export class InsertDataService {
    ...
    insertData(nodeId: number,nodeType:number, editor: any) {
        ...
        const url: string = this.httpData.Manage_HOST + 'pageDesignQueryFacade/getPageContent?id=' + nodeId+'&uid='+userinfo.id;
        this.http.get(url)
            .map(res => res.json())
            .subscribe((res) => {
                    ...
                    for (let i = 0; i < fields.length; i++) {
                        ...
                        switch (dataType) {
                            ...
                            case 'wxgrouptag':
                              ...
                              break;
                            case 'classify':
                              showData = `<input name="leipiNewField" title="${data.title}" value="${data.value}" cid="${data.cid}" sonid="${data.sonid}" nid="${data.nid}" type="text" leipiplugins="classify"/>`;
                              break;
                             case "onlinehard":
                             case "workword":
                             case "workform":
