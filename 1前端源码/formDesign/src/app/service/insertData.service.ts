...
export class InsertDataService {
  ...
  insertData(nodeId: number, nodeType: number, editor: any, is_public: number, page_classify: number, module_classify: number) {
            ...
            switch (dataType) {
              ...
              case 'select':
                ...
                showData =
                  ...
                  `<select name='${data.name}' datasource='${data.datasource}' datasources='${data.datasources}' datatitle='${data.datatitle}' title='${data.title}' orgwidth='${
                    data.orgwidth
              ...
              case 'flowbtn':
                ...
                showData = `<input orgheight="${data.orgheight}"  orgformid="${data.orgformid}"  orgfontcolor="${data.orgfontcolor}" orgbgcolor="${data.orgbgcolor}" orgurl="${
                  data.orgurl
                }"
              ...
              case 'checkboxs':
                showData =
                  ...
                  `orgheight='${data.orgheight}' orgfontsize='${data.orgfontsize}' style='display:inline-block;width:${data.orgwidth};height:${data.orgheight};font-size:${
                    data.orgfontsize
                  }' ` +
                  ` name='${data.name}'>`;
                $.each(data.options, function(key, value) {
                ...
                break;
							...
							case 'reportcontrol':
								showData =
									...
									`style='width: 150px;height: 30px;border:1px solid #ccc;' scoresourceid='${data.scoresourceid}' scoresourcename='${
										data.scoresourcename
									}' tps='${data.tps}' statisticstheme='${data.statisticstheme}' statisticalmaterials='${data.statisticalmaterials}'>`;
								break;
              ...
              case 'group':
                showData +=
                  ...
                  `<input  name="leipiNewField" class="search-query" type="text"  title="${data.title}" value="${data.value}" datasource="${data.datasource}" leipiplugins="${
                    data.leipiplugins
                  }" orghide="${data.orghide}" orgthide="${data.orgthide}" orgbghide="${data.orgbghide}" orgalign="${
                    data.orgalign
                  ...
                  `<tr><td>` +
                  `<div class="wifiblockBox"><div class="wifiblock" groupcon="${data.groupcon}" orgvalue="${data.gValue}" orgTitle="${
                    data.gTitle
                  }" orgurl="/editor/assets/ueditor/formdesign/preview.html?link=${data.orgfUrl}" style="display: flex;display: -webkit-flex;color: #333;">` +
                  `<div class="wfImg" ><input groupcon="${data.groupcon}" name="leipiNewField"  type="text" title="${
                    data.title
                  }" style="display: block;width: 100%;height: 100%;" value="${data.value}" datasource="${data.datasource}" leipiplugins="${data.leipiplugins}"></div>` +
                  `<div class="wfDetail">` +
                  `<div class="title"><input groupcon="${data.groupcon}" name="leipiNewField" type="text" title="${data.title}" value="${data.title}" datasource="${
                    data.datasource
                  }" leipiplugins="${data.leipiplugins}" orghide="${data.orgthide}" orgthide="${data.otgthide}" orgbghide="${data.orgbghide}" orgalign="${
                    data.orgalign
                  }" orgwidth="${data.orgwidth}" orgtype="${data.orgtype}" style="border: 1px solid; text-align: left; width: 150px;"/></div>` +
                  `<div class="detail"><input groupcon="${data.groupcon}" title="${data.title}" name="leipiNewField" leipiplugins="${data.leipiplugins}" value="${
                    data.value
                  }" orgrich="${data.orgrich}" datasource="${data.datasource}" orgfontsize="${data.orgfontsize}" orgwidth="${data.orgwidth}" orgheight="${
                    data.orgheight
                  }" style="width:300px;height:80px; "/></div>` +
                  `<div class="autor"><input groupcon="${
                    data.groupcon
                  }" class="title" name="leipiNewField" type="text" title="文本框" value="" datasource="" leipiplugins="text" orghide="1" orgthide="0" orgbghide="1" orgalign="left" orgwidth="150" orgtype="text" style="border: 1px solid; text-align: left;"/></div>` +
                  `<div class="confscource"><p groupcon="${
                    data.groupcon
                  }" class="title" name="leipiNewField" type="text" title="文本框" value="" datasource="" leipiplugins="text" orghide="1" orgthide="0" orgbghide="1" orgalign="left" orgwidth="150" orgtype="text" style="border: 1px solid; text-align: left; width: 150px;"/></div>` +
              ...
              case 'button':
                showData =
                  ...
                  `orgheight='${data.orgheight}' orgwidth='${data.orgwidth}' orgfontcolor='${data.orgfontcolor}' orgurl='${data.orgurl}' formnode='${data.formnode}' btntype='${
                    data.btntype
                  }' tgnode='${data.tgnode}' winname='${data.winname}' fontsize='${data.fontsize}'>`;
                break;
              case 'getuserid':
              ...
              case 'pagedetails':
                showData = `<div class='wifiblockBoxs' style='border: 1px solid #ddd;'>
                                ...
                                <div data-allowjump='false' class='wifiblock books_con' name='leipiNewField' leipiplugins='wifiblock' orgUrlName="${data.orgUrlName}" orgUrlId="${
                  data.orgUrlId
                }" orgvalue="" orgTitle="" orgurl="/editor/assets/ueditor/formdesign/preview.html?link=${data.orgfUrl}" style='padding: 8px;border-bottom: 1px solid #ddd;'>
                                ...
                                <div data-allowjump='false' class='wifiblock audio_con' name='leipiNewField' leipiplugins='wifiblock' orgUrlName="${data.orgUrlName}" orgUrlId="${
                  data.orgUrlId
                }" orgvalue="" orgTitle="" orgurl="/editor/assets/ueditor/formdesign/preview.html?link=${data.orgfUrl}" style='padding: 8px;border-bottom: 1px solid #ddd;'>
                                ...
                                <div data-allowjump='false' class='wifiblock video_con' name='leipiNewField' leipiplugins='wifiblock' orgUrlName="${data.orgUrlName}" orgUrlId="${
                  data.orgUrlId
                }" orgvalue="" orgTitle="" orgurl="/editor/assets/ueditor/formdesign/preview.html?link=${data.orgfUrl}" style='padding: 8px;border-bottom: 1px solid #ddd;'>
              ...
              case 'routerclassify':
                ...
                break;
              case 'glc':
                showData = `<input name="${data.name}" type="text" title="管理条控件" value="管理 - 控件" leipiplugins="glc" nodeid="${
                  data.nodeid
                }" style="width: 80px;height: 30px;">`;
                break;
              case 'tpl':
                showData = `<input home="${data.home}" site="${data.site}" work="${data.work}" msg="${data.msg}" workbench="${data.workbench}" personal="${
                  data.personal
                }" choicetpl="${data.choicetpl}" tpltype="${data.tpltype}" isshow="${
                  data.isshow
                }" name="leipiNewField" type="text" title="tpl" value="tpl" leipiplugins="tpl" style="height: 20px;width: 20px;border-radius: 8px;" />`;
                break;
              case 'mtpl':
                showData = `<input home=${data.home} site=${data.site} work=${data.work} msg=${
                  data.msg
                } name="leipiNewField" type="text" title="mtpl" value="mtpl" leipiplugins="mtpl" style="height: 20px;width: 20px;border-radius: 8px;" />`;
                break;
              case 'daydate':
                showData = `<input name="${data.name}" dateid="${data.dateid}" orgwidth="${data.orgwidth}" orgheight="${data.orgheight}" orgfontsize="${
                  data.orgfontsize
                }" placeholder="请输入日期" type="date" title="${data.title}" class="date_input" leipiplugins="daydate" style="width:${data.orgwidth};height:${
                  data.orgheihgt
                };font-size:${data.orgfontsize}" />`;
                break;
              case 'timedate':
                showData = `<input name="${data.name}" placeholder="${data.placeholder}" dateid="${data.dateid}" type="${data.type}" title="${data.title}" leipiplugins="${
                  data.leipiplugins
                }" orgwidth="${data.orgwidth}" orgheight="${data.orgheight}" orgfontsize="${data.orgfontsize}" style="width:${data.orgwidth}; height:${data.orgheight};font-size:${
                  data.orgfontsize
                }" />`;
                break;
              case 'iconrouter':
                showData = `<input placeholder="${data.orgtitle}" name="leipiNewField" leipiplugins="button" mode="${data.mode}" orderby="${data.orderby}" orgtitle="${
                  data.orgtitle
                }" orgsrc="${data.orgsrc}" orgbgcolor="${data.orgbgcolor}" orgname="${data.orgname}" orgheight="${data.orgheight}" orgwidth="${data.orgwidth}" orgfontcolor="${
                  data.orgfontcolor
                }" orgurl="${data.orgurl}" formnode="${data.formnode}" btntype="${data.btntype}" tgnode="${data.tgnode}" winname="${data.winname}">`;
                break;
              case 'formlist':
              	showData = `<input personage_info="${data.personage_info}" datasource="${data.datasource}" datafield="${data.datafield}" headshow="${
									data.headshow
								}" tpl="${data.tpl}" pagesize="${data.pagesize}" showpage="${data.showpage}" frmedit="${data.frmedit}" frmdel="${data.frmdel}" frmexa="${
									data.frmexa
								}" style="${data.style}" name="${data.name}" listpich="${data.listpich}" listpicw="${data.listpicw}" url="${data.url}" searchcon='${
									data.searchcon
								}'  type="text" title="${data.title}" value="${data.title}" listfontsize="${data.listfontsize}" seardelval="${data.seardelval}" listheight="${
									data.listheight
								}" searchbyidorname="${data.searchbyidorname}" center="${data.center}" theaderbackground="${data.theaderbackground}" leipiplugins="formlist" />`;
								break;
              case 'meetinglist':
                showData = `<input name="${data.name}" leipiplugins="${data.leipiplugins}" type="${data.type}" value="${data.value}" title="${data.title}" datasource="${
                  data.datasource
                }" datafield="${data.datafield}" pagesize="${data.pagesize}" headshow="${data.headshow}" tpl="${data.tpl}" showpage="${data.showpage}" listpicw="${
                  data.listpicw
                }" listpich="${data.listpich}" roomurl="${data.roomurl}" listfontsize="${data.listfontsize}" searchcon="${data.searchcon}" seardelval="${data.seardelval}" mtype="${
                  data.mtype
                }" mtitle="${data.mtitle}" style="${data.style}"/>`;
                break;
              case 'scheduling':
                showData = `<input datasource="${data.datasource}" datafield="${data.datafield}" headshow="${data.headshow}" tpl="${data.tpl}" pagesize="${
                  data.pagesize
                }" showpage="${data.showpage}" frmedit="${data.frmedit}" frmdel="${data.frmdel}" style="${data.style}" name="${data.name}" listpich="${data.listpich}" listpicw="${
                  data.listpicw
                }"  type="text" title="${data.title}" value="${data.value}" leipiplugins="scheduling" />`;
                break;
              case 'wxlist':
                showData = `<input name="${data.name}" type="text" title="微信短信列表" value="微信短信列表" leipiplugins="wxlist" style="${data.style}" msgflag="${
                  data.msgflag
                }" wxsendtype="${data.wxsendtype}" pagesize="${data.pagesize}"/>`;
                break;
              case 'listone':
              	showData = `<input isnotuser="${data.isnotuser}" name="${data.name}" leipiplugins="listone" type="text" value="${data.value}" title="${
									data.title
								}" datasource="${data.datasource}" datapic="${data.datapic}" datatitle="${data.datatitle}" datacontent="${data.datacontent}" datacontent2="${
									data.datacontent2
								}" pagesize="${data.pagesize}" listpich="${data.listpich}" listpicw="${data.listpicw}" url="${
									data.url
								}" style="height: 20px;line-height: 20px;" bottomline="${data.bottomline}" showpage="${data.showpage}" orderby="${data.orderby}" colnumber="${
									data.colnumber
								}" tpl="${data.tpl}" titlesize="${data.titlesize}" consize="${data.consize}" titlelabel="${data.titlelabel}" conlabel="${
									data.conlabel
								}" typesevendate="${data.typesevendate}" frmedit="${data.frmedit}" fontsize="${data.fontsize}" frmdel="${data.frmdel}" searchcon="${
									data.searchcon
								}" seardelval="${data.seardelval}" picfontsize="${data.picfontsize}" listheight="${data.listheight}" prefix="${data.prefix}" isheader="${
									data.isheader
								}" headertitle="${data.headertitle}" moreurl="${data.moreurl}" headercolor="${data.headercolor}" titlecolor="${data.titlecolor}" iscomment="${
									data.iscomment
								}" outborder="${data.outborder}" prefixcolor="${data.prefixcolor}" headstyle="${data.headstyle}" headimgsrc="${data.headimgsrc}" pagingstyle="${
									data.pagingstyle
								}" isscore="${data.isscore}" onlyshowone="${data.onlyshowone}" multisource="${data.multisource}" multititles="${
									data.multititles
								}" multisourcenames="${data.multisourcenames}" multinodeids="${data.multinodeids}" multidatapics="${data.multidatapics}" multidatatitles="${
									data.multidatatitles
								}" multidatacontents="${data.multidatacontents}" multititlesizes="${data.multititlesizes}" multiconsizes="${
									data.multiconsizes
								}" multititlelabels="${data.multititlelabels}" multiconlabels="${data.multiconlabels}" multifontsizes="${data.multifontsizes}" multilistpicws="${
									data.multilistpicws
								}" multilistpichs="${data.multilistpichs}" multilisturls="${data.multilisturls}" multisearchcons="${data.multisearchcons}" multiseardelvals="${
									data.multiseardelvals
								}" multipicfontsizes="${data.multipicfontsizes}" multilistheights="${data.multilistheights}" multirowsourcecount="${
									data.multirowsourcecount
								}" multisourcepublic="${data.multisourcepublic}"   typesevenurl="${data.typesevenurl}"   iscolor="${data.iscolor}"/>`;

                break;
              case 'classify':
                showData = `<input name="${data.name}" title="${data.title}" value="${data.value}" cid="${data.cid}" sonid="${data.sonid}" nid="${
                  data.nid
                }" type="text" classtype="${data.classtype}" leipiplugins="classify" datasource="${data.datasource}" datafiled="${data.datafiled}" />`;
                break;
              case 'wxgrouptag':
                showData = `<input name="${data.name}" type="text" title="微信分组标签" value="微信分组标签" msgflag="${
                  data.msgflag
                }" leipiplugins="wxgrouptag" style="width: 90px;height: 30px;"/>`;
                break;
              case 'settimeout':
                showData = `<input orgtime="${data.orgtime}" orgwidth="${data.orgwidth}" orgheight="${data.orgheight}" name="leipiNewField" type="text" color="${
                  data.color
                }" title="${data.title}" value="${data.title}" orgfontsize="${data.orgfontsize}" leipiplugins="settimeout" style="${data.style}"/>`;
                break;
              case 'searchcontrol':
                showData = `<input name="${data.name}" searchName="${data.searchname}" searchContent="${data.searchcontent}" type="text" leipiplugins="${
                  data.leipiplugins
                }" style="height: 20px;width: 20px;border-radius: 8px;"
                                searchFieldText="${data.searchfieldtext}" searchFieldValue="${data.searchfieldvalue}" prdId="${data.prdid}" nodeId="${data.nodeid}" orgwidth="${
                  data.orgwidth
                }" orgheight="${data.orgheight}"
                               searchKeyDataText="${data.searchkeydatatext}" searchKeyDataValue="${data.searchkeydatavalue}" orgtitle="${data.orgtitle}" orgchecked="${
                  data.orgchecked
                }"/>`;
                break;
              case 'buylabel':
                showData = `<input name="${data.name}" leipiplugins="buylabel" mode="${data.mode}" orgtitle="${data.orgtitle}" orgsrc="${data.orgsrc}" value="${
                  data.value
                }" orgheight="${data.orgheight}" orgwidth="${data.orgwidth}" orgfontsize="${data.orgfontsize}" orgbgcolor="${data.orgbgcolor}" orgfontcolor="${
                  data.orgfontcolor
                }" orgfurl="${data.orgfurl}" orgurl="${data.orgurl}"/>`;
                break;
              case 'popup':
                showData = `<input name="${data.name}" leipiplugins="popup" mode="${data.mode}" orgtitle="${data.orgtitle}" orgsrc="${data.orgsrc}" value="${
                  data.value
                }" orgheight="${data.orgheight}" orgwidth="${data.orgwidth}" orgfontsize="${data.orgfontsize}" orgbgcolor="${data.orgbgcolor}" orgfontcolor="${
                  data.orgfontcolor
                }" orgfurl="${data.orgfurl}" orgurl="${data.orgurl}"/>`;
              ...
              case 'lunbo':
                showData = `<input name="${data.name}" leipiplugins="${data.leipiplugins}" type="${data.type}" value="${data.value}" title="${data.title}" datasource="${
                  data.datasource
                }" limtvalue="${data.limtvalue}" datapic="${data.datapic}" datatitle="${data.datatitle}" dataurl="${data.dataurl}" orgwidth="${data.orgwidth}" orgheight="${
                  data.orgheight
                }" client="${data.client}" wtime="${data.wtime}" />`;
                break;
              case 'addnumber':
                showData = `<input name="${data.name}" leipiplugins="${data.leipiplugins}" mode="${data.mode}" orgtitle="${data.orgtitle}" orgsrc="${data.orgsrc}" value="${
                  data.value
                }" addvalue="${data.addvalue}" orgheight="${data.orgheight}" orgwidth="${data.orgwidth}" orgfontsize="${data.orgfontsize}" orgbgcolor="${
                  data.orgbgcolor
                }" orgfontcolor="${data.orgfontcolor}" datasource="${data.datasource}" datafiled="${data.datafiled}"/>`;
                break;
              case 'recordlabel':
                showData = `<input name="${data.name}" leipiplugins="recordlabel" type="text" value="${data.value}" title="${data.title}" datasource="${
                  data.datasource
                }" datatitle="${data.datatitle}"/>`;
                break;
              case 'background':
                showData = `<input name="${data.name}" leipiplugins="background" type="text" value="${data.value}" title="${data.title}" bagcolor="${data.bagcolor}" bagpic="${
                  data.bagpic
                }"/>`;
                break;
              case 'export':
                showData = `<input name="${data.name}" leipiplugins="export" type="text" value="${data.value}" title="${data.title}" datasource="${data.datasource}" datafield="${
                  data.datafield
                }" pluwidth="${data.pluwidth}" pluheight="${data.pluheight}" fontsize="${data.fontsize}" bgcolor="${data.bgcolor}" fontcolor="${data.fontcolor}" style="${
                  data.style
                }"/>`;
                break;
              case 'shenpi':
                showData = `<input name="${data.name}" leipiplugins="shenpi" type="text" value="${data.value}" title="${data.title}" datasource="${data.datasource}" datafiled="${
                  data.datafiled
                }" spstate="${data.spstate}" pluheight="${data.pluheight}" fontsize="${data.fontsize}" spdel="${data.spdel}" spdbox="${data.spdbox}" style="width: 80px;"/>`;
                break;
              case 'internal':
                ...
                break;
              case 'exchange':
                showData = `<input name="${data.name}" leipiplugins="exchange" mode="${data.mode}" orgtitle="${data.orgtitle}" orgsrc="${data.orgsrc}" value="${
                  data.value
                }" orgheight="${data.orgheight}" orgwidth="${data.orgwidth}" orgfontsize="${data.orgfontsize}" orgbgcolor="${data.orgbgcolor}" orgfontcolor="${
                  data.orgfontcolor
                }" orgfurl="${data.orgfurl}" orgurl="${data.orgurl}"/>`;
              ...
              case 'chaxun':
              case 'audit':
                showData = `<input width="${data.orgwidth}" height="${data.orgheight}"  home=${
                  data.home
                }  name="leipiNewField" type="text" title="mtpl" value="mtpl" leipiplugins="mtpl" style="height: 20px;width: 20px;border-radius: 8px;" />`;
                break;
              default:
